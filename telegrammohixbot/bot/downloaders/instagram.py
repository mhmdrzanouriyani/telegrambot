"""
Instagram Reels and public posts downloader using yt-dlp.

This uses the yt-dlp Python API (no browser automation, no external services).
"""
...
def _build_ydl_opts(download_dir: Path) -> Dict[str, Any]:
    """
    Build yt-dlp options for Instagram reels and posts.
...
def _download_sync(url: str, download_dir: Path) -> Path:
    ...
    logger.info("Starting Instagram download for url=%s", url)
    ...
    logger.info("Instagram download finished: %s", downloaded_file)
    return downloaded_file


async def download_instagram_video(url: str, download_dir: Path) -> Path:
    """
    Download an Instagram reel or public post to the given directory.
